...
const main = 'runtime/tsc3.js'
// Real, checked, not assumed: every lo.builtin() call in tsc2.js
// (require()'s load of runtime/typescript.js, small.ts/big.ts, the lib
// .d.ts files, console.d.ts) runs only at tsc2.js's own top level -
// none inside snapshotEntry - so none of libs/embeds is ever read again
// once the snapshot exists (grepped directly, 2026-08-27). V8's
// serializer doesn't strip a Script's source string either (checked
// against real v8/src/snapshot/serializer.cc - only line-ends/
// compiled-lazy-positions caches get cleared), so this is safe even for
// any TypeScript-internal function never exercised during the build's
// warm-up compile. See lib/build.js's post_snapshot_embeds doc comment.
