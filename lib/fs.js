const { assert, readMemory, utf8Decode, wrap, core } = lo
const { 
  closedir, unlink, rmdir, open, fstat, close, access, mkdir, strnlen,
  F_OK, O_RDONLY, S_IFMT, S_IFDIR, S_IFREG,
  S_IRWXU, S_IRWXG, S_IROTH, NAME_MAX
} = core
...
let MODE_WORD = 6
if (core.arch === 'arm64') MODE_WORD = 4
...
function isFile (path) {
  ...
  close(fd)
  return checkMode(stat32[MODE__WORD], S_IFREG)
}
...
function isDir (path) {
  ...
  if (r < 0) return false
  close(fd)
  return checkMode(stat32[MODE__WORD], S_IFDIR)
}
