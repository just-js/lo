...
const { utf8EncodeIntoAtOffset, addr, assert, ptr } = lo
...
net.constants = {
  ...
  SOCKADDR_LEN, SOCKADDR_LEN_UNIX, SOMAXCONN, O_CLOEXEC, SOCK_CLOEXEC, MSG_NOSIGNAL, PF_PACKET,
...
function sockaddr_un (path) {
  const u8 = ptr(new Uint8Array(MAX_UNIX_PATH + 2))
  const dv = new DataView(u8.buffer)
  dv.setInt16(0, AF_UNIX, true)
  utf8EncodeIntoAtOffset(path, u8.ptr, 2)
  return u8
}

net.pipe = (pipes, flags = O_CLOEXEC) => {
  const fds = ptr(new Uint32Array(2))
  const rc = net.pipe2(fds.ptr, flags)
  if (rc === 0) {
    pipes[0] = fds[0]
    pipes[1] = fds[1]
  }
...
const off = ptr(new Uint32Array([0]))
