const api = {
  ...
  EVP_get_digestbynid: {
    // Real symbol is lo_EVP_get_digestbynid (see preamble below):
    // EVP_get_digestbynid is a function-like macro in this OpenSSL's
    // headers (`#define EVP_get_digestbynid(a)
    // EVP_get_digestbyname(OBJ_nid2sn(a))`), not an addressable function
    // -- `(void*)EVP_get_digestbynid` is an undeclared-identifier error
    // because a function-like macro only expands when followed by `(`.
    // Fine to call directly (with real args) under the v8 target's own
    // dispatch, but the abi target's dispatch takes the function's
    // address, so it needs a real wrapper symbol. `name` here (not the
    // exported key) is what both codegens use as the actual C symbol --
    // see lib/gen.js's `fn.name = fn.name || fname`.
    name: 'lo_EVP_get_digestbynid',
    parameters: ['i32'],
    result: 'pointer',
  ...
  X509_set_issuer_name: {
    parameters: ['pointer', 'pointer'],
    pointers: ['X509*', 'const X509_NAME*'],
    result: 'i32'
  },
  SSL_CTX_set_verify: {
    parameters: ['pointer', 'i32', 'pointer'],
    pointers: ['SSL_CTX*', , 'SSL_verify_cb'],
    result: 'void'
  },
  SSL_CTX_set_read_ahead: {
    // Same macro-not-a-function issue as EVP_get_digestbynid above --
    // see lo_SSL_CTX_set_read_ahead in the preamble.
    name: 'lo_SSL_CTX_set_read_ahead',
    parameters: ['pointer', 'i32'],
    pointers: ['SSL_CTX*'],
    result: 'void'
  },
  SSL_CTX_set_mode: {
    // Same macro-not-a-function issue -- see lo_SSL_CTX_set_mode in the
    // preamble.
    name: 'lo_SSL_CTX_set_mode',
    parameters: ['pointer', 'u64'],
    pointers: ['SSL_CTX*'],
    result: 'i32'
  },
  // Needed for subjectAltName (SAN) extensions on generated certs -- see
  // generate_cert() in lib/libssl.js. Modern browsers (Chrome 58+) ignore
  // the legacy CN field entirely for hostname validation and require SAN.
  X509V3_set_ctx: {
    parameters: ['pointer', 'pointer', 'pointer', 'pointer', 'pointer', 'i32'],
    pointers: ['X509V3_CTX*', 'X509*', 'X509*', 'X509_REQ*', 'X509_CRL*'],
    result: 'void'
  },
  X509V3_EXT_nconf_nid: {
    parameters: ['pointer', 'pointer', 'i32', 'pointer'],
    pointers: ['CONF*', 'X509V3_CTX*', , 'const char*'],
    result: 'pointer',
    rpointer: 'X509_EXTENSION*'
  },
  X509_add_ext: {
    parameters: ['pointer', 'pointer', 'i32'],
    pointers: ['X509*', 'X509_EXTENSION*'],
    result: 'i32'
  },
  X509_EXTENSION_free: {
    parameters: ['pointer'],
    pointers: ['X509_EXTENSION*'],
    result: 'void'
  }
}
...
//const obj = ['/lib/x86_64-linux-gnu/libssl.a', '/lib/x86_64-linux-gnu/libcrypto.a']
//const libs = []
...
if (globalThis.lo) {
  ...
  if (lo.core.os === 'mac') {
    include_paths.push(`${PREFIX}/openssl@3.0/include`)
    lib_paths.push(`${PREFIX}/openssl@3.0/lib`)
    // add these if we want to statically link libssl into the shared library
    obj.push(`${PREFIX}/openssl@3.0/lib/libssl.a`)
    obj.push(`${PREFIX}/openssl@3.0/lib/libcrypto.a`)
  } else if (lo.core.os === 'linux') {
    // dynamic link to system libssl
//    libs.push('ssl')
//    libs.push('crypto')
    // static link the one we built
    obj.push('deps/openssl/libssl.a')
    obj.push('deps/openssl/libcrypto.a')
  }
}
*/
...
const mac = {
  libs: ['ssl', 'crypto'],
  lib_paths: ['openssl@3/lib'],
  include_paths: ['openssl@3/include'],
  prefix: '/opt/homebrew/opt/'
}

// Real, addressable wrappers for OpenSSL entries that are function-like
// macros in this OpenSSL's headers, not actual functions -- see the
// matching comments on EVP_get_digestbynid/SSL_CTX_set_read_ahead/
// SSL_CTX_set_mode above for why a wrapper is needed at all.
const preamble = `
static const EVP_MD* lo_EVP_get_digestbynid(int nid) {
  return EVP_get_digestbynid(nid);
}
static void lo_SSL_CTX_set_read_ahead(SSL_CTX* ctx, int m) {
  SSL_CTX_set_read_ahead(ctx, m);
}
static int lo_SSL_CTX_set_mode(SSL_CTX* ctx, uint64_t op) {
  return (int)SSL_CTX_set_mode(ctx, (long)op);
}
`

export { api, name, includes, libs, obj, constants, include_paths, lib_paths, preamble, linux, mac }
