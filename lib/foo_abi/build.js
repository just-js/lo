...
// pattern). Unlike lib/ada/build.js's vendored third-party source
// (fetched once, never touched again -- skip-if-exists is correct there),
// lo_abi_v8.cc is the file this ABI prototype actively iterates on, so it
// is recompiled unconditionally here every time -- a skip-if-exists check
// silently kept serving a stale .o across edits (real bug, hit directly:
// edits to lo_abi_v8.cc had zero effect on foo_abi.so until the stale .o
// was manually deleted first). It's one small file; recompiling always is
// cheap enough that the staleness risk isn't worth the check.
//
// compile_bindings (lib/build.js) passes its own CFLAGS as the 3rd arg
// here rather than us importing it -- lib/build.js is still mid-
// evaluation (inside compile_bindings) when it dynamically imports this
// file, so `import { CFLAGS } from 'lib/build.js'` here is a real
...
async function build (CC = 'gcc', CXX = 'g++', CFLAGS = []) {
  ...
  if ((bindings.obj || []).length) {
    const status = new Int32Array(3)
    const CXXARGS = CXX.split(' ')
    exec(CXXARGS[0], [
      ...CXXARGS.slice(1), ...CFLAGS,
      '-O3', '-march=native', '-mtune=native', '-fno-stack-protector', '-fno-optimize-sibling-calls', 
