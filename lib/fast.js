...
const { fastcall, bind_fastcall, bind_slowcall } = spin.load('fast').fast
const {
  rax, rbx, rcx, rdx, rsi, rdi, rbp, rsp, r8, r9, r10
} = Registers
const { ptr, dlsym } = spin
...
function bindall (config, handle = 0) {
  const mod = {}
  for (const key of Object.keys(config)) {
    const binding = {}
    const { constants, bindings, native = {} } = config[key]
    Object.assign(binding, constants)
    for (const key of Object.keys(bindings)) {
      const { name, result, parameters } = bindings[key]
      binding[key] = bind(dlsym(handle, name || key), result, parameters)
    }
