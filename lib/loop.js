...
const {
  ...
  EPOLL_CTL_MOD, EAGAIN, struct_epoll_event_size
} = epoll

function event (fd, mask = EPOLLIN | EPOLLOUT) {
  const buf = new ArrayBuffer(struct_epoll_event_size)
...
function events (nevents = 1024) {
  const buf = new ArrayBuffer(nevents * struct_epoll_event_size)
