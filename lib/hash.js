...
const { assert, utf8Length, ptr } = spin
...
class Digest {
  ...
  hash (buf) {
    ...
    assert(EVP_DigestUpdateBuffer(ctx, buf, buf.size) === 1)
    assert(EVP_DigestFinal(ctx, digest, hsize) === 1)
    EVP_MD_CTX_reset(ctx)
    return digest
  }
  ...
  hashString2 (str) {
    const { hasher, hsize, digest, ctx } = this
    assert(EVP_DigestInit_ex(ctx, hasher, 0) === 1)
    assert(EVP_DigestUpdate(ctx, str.ptr, str.size) === 1)
    assert(EVP_DigestFinal(ctx, digest, hsize) === 1)
    EVP_MD_CTX_reset(ctx)
    return digest
  }
