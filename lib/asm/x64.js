...
class Assembler {
  ...
  zero (reg) {
    ...
    if (reg === 'rax') {
      ...
      this.#codes.push([0x48, 0x31, 0xd2])
    } else if (reg === 'rdi') {
  ...
  cmp (reg, imm) {
    ...
    //              2: imm8/16/32
    const idx = gp[reg]
    if (imm < pow32 && reg === 'rax') {
      this.#instr.push(`cmp %eax, ${imm}`)
      this.#codes.push([op_cmp, ...as_four_bytes(imm)])
      return this
    }
    this.#instr.push(`cmp %${reg}, ${imm}`)
    this.#codes.push([rex(1, 0, 0, idx < 8 ? 0 : 1), op_cmp + (idx % 8), 
      ...as_four_bytes(imm)])
    return this
  }
  ...
  je (addr) {
    if (addr < pow8) {
      this.#instr.push(`je :${addr}`)
      this.#codes.push([op_je8, addr])
      return this
    }
    this.#instr.push(`je :${addr}`)
    this.#codes.push([...op_je32, ...as_four_bytes(addr)])
    return this
  }
  ...
  disassemble (name) {
    ...
    let pos = 0
    const lines = []
    lines.push(`${AY}<${AM}${name}${AY}>${AD}:\n`)
    for (let i = 0; i < insts.length; i++) {
      const inst = insts[i]
      const code = codes[i]
      const size = code.length
      if (size === 0) {
        lines.push(`${AM}${inst}${AD} :`)
      } else {
        let op = inst
        let rest = ''
        const first_space = inst.indexOf(' ')
        if (first_space > -1) {
          op = inst.slice(0, first_space)
          rest = inst.slice(first_space + 1, inst.length)
        }
        lines.push(`${AG}${pos.toString(16).padStart(4, ' ')}${AD}:  ${AY}${code.map(n => n.toString(16).padStart(2, '0')).join(' ').padEnd(56, ' ')}${AD} ${AC}${op.padEnd(10, ' ')}${AD} ${rest}`)
      }
      pos += size
    }
    return lines
