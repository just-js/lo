...
// https://developer.arm.com/documentation/ddi0602/2024-12/Base-Instructions/LDR--immediate---Load-register--immediate--
function encode_ldr (src, dest, off = 0) {
  ...
  const bits = [
    ...
    abs_off_8 >> 11 & 0x01,                                // 21
    abs_off_8 >> 10 & 0x01,                                // 20
    abs_off_8 >> 9 & 0x01,                                // 19
    abs_off_8 >> 8 & 0x01,                                // 18
    abs_off_8 >> 7 & 0x01,                                // 17
    abs_off_8 >> 6 & 0x01,                                // 16
...
function encode_stp (src1, src2, dest, off = 0) {
  // pre-index
  const abs_off_8 = off !== 0 ? Math.abs(off) / 8 : 0
  const twos_complement = off < 0 ? (~abs_off_8 + 1) : abs_off_8
  const bits = [
    1, // 31
    0, // 30
    1, // 29
...
//    off < 0 ? 1 : 0,                                // 21
    twos_complement >> 6 & 0x01,
    twos_complement >> 5 & 0x01,
    twos_complement >> 4 & 0x01,
    twos_complement >> 3 & 0x01,
    twos_complement >> 2 & 0x01,
    twos_complement >> 1 & 0x01,
...
function encode_ldp (dest1, dest2, src, off = 0) {
  ...
  const bits = [
    ...
    1, // 23
    1, // 22
    twos_complement >> 6 & 0x01, // 21
    twos_complement >> 5 & 0x01, // 20

    twos_complement >> 4 & 0x01, // 19
    twos_complement >> 3 & 0x01, // 18
    twos_complement >> 2 & 0x01, // 17
    twos_complement >> 1 & 0x01, // 16

    twos_complement & 0x01, // 15
    dest2 >> 4 & 0x01,                                // 14
    dest2 >> 3 & 0x01,                                // 13
    dest2 >> 2 & 0x01,                                // 12
...
function encode_stri (src, dest, off = 0) {
  const bits = [
    1, // 31
//    off < pow32 ? 0 : 1, // 30
    1,
...
function encode_strr (src, base, index, off) {
  const bits = [
    1, // 31
    off < pow32 ? 0 : 1, // 30
    1, // 29
...
class Assembler {
  ...
  stri (src, dest, off = 0) {
    ...
    this.codes.push(encode_stri(src_idx, dest_idx, off))
    return this
  }

  strr (src, base, index, off = 0) {
    const src_idx = gp64[src]
    const base_idx = gp64[base]
    const index_idx = gp64[index]
    this.#instr.push(`str ${src}, [${base}, (${index})]`)
    this.codes.push(encode_strr(src_idx, base_idx, index_idx, off))
    return this
  }
  ...
  disassemble (name) {
    ...
    for (let i = 0; i < insts.length; i++) {
      ...
      const first_space = inst.indexOf(' ')
      const op = inst.slice(0, first_space)
      const rest = inst.slice(first_space + 1, inst.length)
      lines.push(`${pos.toString(16).padStart(4, ' ')}:  ${code.map(n => n.toString(16).padStart(2, '0')).join(' ').padEnd(30, ' ')}  ${op.padEnd(8, ' ')} ${rest}`)
      pos += size
    }
    return lines
  }  
