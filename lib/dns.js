...
const default_bind_ip = lo.getenv('LO_DNS_BIND_IP') || '127.0.0.1'
...
class Resolver {
   ...
   * @returns {void}
   */
  lookup (query = 'localhost', onRecord = (err, ip) => {}, address = '127.0.0.1', port = 53, buf = new Uint8Array(65536), bind_ip = default_bind_ip) {
    if (!buf.ptr) ptr(buf)
...
let resolver
//const resolver = new Resolver(lo.loop)

function lookup (hostname) {
  if (!resolver) resolver = new Resolver(lo.loop)
  return new Promise((resolve, reject) => {
