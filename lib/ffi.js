...
function platform_x64 () {
  ...
  function compile_fastcall (address, result, params, kHasReceiver = true) {
    asm.reset()
...
function bind (address, res = 'void', params = [], slow = false, fast_address = 0, slow_address = 0, kHasReceiver = true) {
  ...
  let fast_src
  if (!slow) {
    const fastcall_target = fast_address ? fast_address : compile_fastcall(address, result, params, kHasReceiver)
  ...
  dv.setUint8(slowcall_pointer_off + 8, kHasReceiver ? 1 : 0)
  // todo: wrap here
...
function bind_custom (res = 'void', params = [], slow_address = 0, fast_address = 0, kHasReceiver = true) {
...
const callback_address = lo.lo_callback_address()
