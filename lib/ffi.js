...
const {
  dlopen, dlsym, getAddress, cstr, assert, dlclose, ptr
} = spin
...
class Library {
  ...
  getSymbol (name) {
    return tcc.tcc_get_symbol(this.compiler, cstr(name).ptr)
  }

  setSymbol (name, sym) {
    return tcc.tcc_add_symbol(this.compiler, cstr(name).ptr, sym)
  }
