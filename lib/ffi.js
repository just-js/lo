...
const { ffi_prep_cif, bindFastApi, bindSlowApi } = ffi
...
const {
  dlopen, dlsym, getAddress, assert, dlclose, ptr
} = spin
...
class FFIFunction {
  constructor (address, rtype, params = []) {
    ...
    this.res = new Uint32Array(2)
    this.cargs = new Uint8Array(8 * params.length)
    this.params = params
    this.rtype = rtype
  ...
  prepare () {
    ...
    const status = ffi_prep_cif(cif, FFI_LAST_ABI, length, rc, args)
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      switch (params[i]) {
        ...
        case Types.u32:
          ...
          break;
        case Types.u8:
          source.push(`  rdv.setUint8(${poff}, p${i});`)
          break;
        case Types.i8:
          source.push(`  rdv.setInt8(${poff}, p${i});`)
          break;
        case Types.u16:
          source.push(`  rdv.setUint16(${poff}, p${i}, true);`)
          break;
        case Types.i16:
          source.push(`  rdv.setInt16(${poff}, p${i}, true);`)
          break;
      }
      poff += psize
    }
    source.push('  ffi_call(cif, address, res, cargs)')
    ...
    this.call = f(cif, address, rdv, getAddress, ffi.ffi_call, res, cargs, P32, strings, strptrs)
    return this
  }
}
...
function CParamsFromParams2 (params, unwrap = false) {
  let i = 1
  const pp = params.map(t => `${(t === Types.string || t === Types.buffer || t === Types.u32array) ? 'void*' : CArgtypeFromType(t)} p${i++}`)
  if (unwrap) {
    pp.push('struct FastApiTypedArray* const p_ret')
  }
  return pp.join(', ')
...
function genTrampoline (name, rtype, params, extern = true) {
  ...
  if (unwrap) {
    ...
    source.push(`  ${CRtypeFromType(rtype)} r = ${extern ? 'extern_' : ''}${name}(${params.map((t, i) => `p${i + 1}${(t === Types.string || t === Types.buffer || t === Types.u32array) ? '->data' : ''}`).join(', ')});`)
    ...
    source.push(`  return ${extern ? 'extern_' : ''}${name}(${params.map((t, i) => `p${i + 1}${(t === Types.string || t === Types.buffer || t === Types.u32array) ? '->data' : ''}`).join(', ')});`)
  }
  source.push(`}`)
  return source.join('\n')
}
...
class Library {
  ...
  compile (src, objFileName = '') {
    ...
    if (objFileName) {
      ...
      //tcc.tcc_set_options(compiler, '-D__GNUC__=3.45')
      spin.assert(tcc.tcc_compile_string(compiler, src) === 0)
      spin.assert(tcc.tcc_output_file(compiler, objFileName) === 0)
    } else {
  ...
  bind (api) {
    ...
    tcc.tcc_set_output_type(compiler, TCC_OUTPUT_MEMORY)
    //console.log(source.join('\n'))
    let rc = tcc.tcc_compile_string(compiler, source.join('\n'))
    ...
    for (const name of names) {
      ...
      rc = tcc.tcc_add_symbol(compiler, `extern_${name}`, sym)
    ...
    for (const name of names) {
      ...
      const params = def.parameters.map(n => Types[n])
      const rtype = Types[def.result]
      const sym = this.symbols[name] || tcc.tcc_get_symbol(compiler, sname)
      assert(sym)
      const addr = tcc.tcc_get_symbol(compiler, `func_${name}`)
      assert(addr, 'could not get symbol')
      if (def.nofast) {
        lib[name] = bindSlowApi(sym, addr, rtype, params)
      } else {
  ...
  setSymbol (name, sym) {
    return tcc.tcc_add_symbol(this.compiler, name, sym)
  }

  relocate () {
    return tcc.tcc_relocate(this.compiler, TCC_RELOCATE_AUTO)
  }
...
export { 
  Library, Types, FFIFunction, tcc, bindFastApi, bindSlowApi,
  isPointerType, isReturnedAsBigInt, ffitypes, FFIArgtypeFromType
}
