...
tcc.tcc_new = spin.wrap(u32, tcc.tcc_new, 0)
tcc.tcc_get_symbol = spin.wrap(u32, tcc.tcc_get_symbol, 2)
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    switch (params[i]) {
      ...
      case Types.iSize:
        ...
        break;
      case Types.string:
        // todo: reuse single buffer with encodeInto
        source.push(`  if (p${i} !== l${i}) {`)
        source.push(`    rdv.setBigUint64(${poff}, BigInt(spin.cstr(p${i}).ptr), true);`)
        source.push(`    l${i} = p${i}`)
        source.push('  }')
        break;
      case Types.u64:
...
class Library {
  ...
  bind (api) {
    ...
    for (const name of names) {
      ...
      if (needsUnwrap(rtype)) {
        lib[def.name || name] = spin.wrap(new Uint32Array(2), bindFastApi(addr, Types.void, [...params, Types.u32array], ffi_func), def.parameters.length)
        //lib[def.name || name] = bindFastApi(addr, Types.void, [...params, Types.u32array], ffi_func)
        lib[def.name || name].slow = ffi_func
        continue
      }
...
export { Library, Types, wrapffi, tcc }
