...
async function create_lo_home () {
  ...
  const v8_dir = join(LO_HOME, 'v8')
  if (!is_dir(v8_dir)) {
    console.log(`${AM}creating${AD} ${v8_dir}`)
    mkdir_all_safe(`${v8_dir}`)
    assert(chdir(v8_dir) === 0)
    const file_name = 'include.tar.gz'
    console.log(`${AY}download v8 includes for version ${AD}${v8}`)
    const size = fetch(`${v8_url_prefix}/${v8}/include.tar.gz`, file_name)
    assert(size > 0)
    ...
    assert(chdir(cwd) === 0)
    assert(is_dir(v8_dir))
    assert(is_dir(join(v8_dir, 'include')))
  }
  const v8_lib = join(LO_HOME, `v8/libv8_monolith.a`)
  if (!is_file(v8_lib)) {
    console.log(`${AM}creating${AD} ${v8_lib}`)
...
// add a --vscode build flag and a --git build flag and a --npm build flag
// these will create .vscode settings for building and debugging
// --git will create a .gitignore
// --npm will create a package.json and node_modules with lo types. maybe it should be --types
// --make will create a makefile to build your app
// --bash will create a bash script to build your app
// --docker will create a dockerfile to build a container from your app
// we could create a tiny builder version of lo in docker - statically linked with just everything you need to build
...
async function install (args = []) {
  ...
  const file_path = join(dest_path, 'lo-tmp')
  const src = read_file(LO_PATH)
  write_file(file_path, src, defaultWriteFlags, S_IRWXU | S_IRWXG | S_IROTH)
  rename(file_path, LO_PATH)
...
const CFLAGS = (getenv('CFLAGS') || `-fPIC -std=c++20 -c -fomit-frame-pointer -fno-rtti -fno-exceptions -DV8_TYPED_ARRAY_MAX_SIZE_IN_HEAP=64 -DV8_ALLOCATION_FOLDING -DV8_SHORT_BUILTIN_CALLS`).split(' ')
