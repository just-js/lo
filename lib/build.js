...
//      write_file(file_path, encoder.encode(lo.builtin(name)))
    }
  }
*/
/*
  ...
  if (!is_file('v8/libv8_monolith.a')) {
    const file_name = `libv8_monolith-${os}-${arch}.a.gz`
    console.log(`${AY}download v8 static lib for version ${AD}${v8}`)
    const size = 
      fetch(`${v8_url_prefix}/${v8}/libv8_monolith-${os}-${arch}.a.gz`, 
    ...
    console.log(`${AY}downloaded${AD} ${file_name} ${AG}size${AY} ${size}`)
    const bytes = read_file(file_name)
    write_file('v8/libv8_monolith.a', inflate(bytes))
    unlink(file_name)
  }
...
- check if we are asking for a binding that is in builtins
- check if binding exists in LO_HOME
...
/*
  ...
  if (!is_dir(lib_dir) && lo.builtins().includes(binding_path)) {
    console.log(`${AM}create dir${AD} ${lib_dir}`)
    assert(mkdir(`${lib_dir}`, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) === 0)
    console.log(`${AM}create bindings def at ${AD} ${lib_dir}/api.js`)
    write_file(`${lib_dir}/api.js`, encoder.encode(lo.builtin(binding_path)))
  }
*/
...
//  config.os = 'win'
//  write_file(`${LO_HOME}/builtins.h`, encoder.encode([`main.js`, ...libs].map(linkerScript).join('')))
  const cwd = getcwd()
...
//  const os = config.os
//  config.os = 'win'
//  write_file(`${LO_HOME}/main_win.h`, encoder.encode(headerFile([...libs, 
//    ...bindings.map(n => `lib/${n}/${n}.a`)], index, main, opts)))
//  config.os = 'mac'
  // todo: this is a hack to get mach in core runtime on macos for now
//  write_file(`${LO_HOME}/main_mac.h`, encoder.encode(headerFile([...libs, 
//    ...bindings.map(n => `lib/${n}/${n}.a`)], index, main, opts)))
//    ...[...bindings.includes('mach') ? [] : ['mach'], ...bindings].map(n => `lib/${n}/${n}.a`)], index, main, opts)))
//  config.os = 'linux'
  write_file(`${LO_HOME}/main.h`, encoder.encode(headerFile([...libs, 
    ...bindings.map(n => `lib/${n}/${n}.a`)], index, main, opts)))
//  config.os = os
}
...
async function build_runtime ({ libs = lo.builtins(), bindings = lo.libraries(), 
  ...
  let lib_paths = await libPaths(bindings.map(n => `lib/${n}/api.js`), { prefix: PREFIX })
  let bin = target
...
async function build (args) {
  ...
  if (action === 'runtime') {
    ...
    if (!is_file(config_name)) {
      //const config = Object.assign({}, await import('runtimes/base.config.js'))
      const config = await import('runtimes/base.config.js')
      //const config = JSON.parse(lo.builtin('runtimes/base.config.js'))
...
async function init (args) {
  const app_name = fileName(lo.getcwd())
  const config = await import('runtimes/core.config.js')
...
${app_name}
`))
  write_file(`jsconfig.js`, encoder.encode(`{
  "compilerOptions": {
    "checkJs": true,
    "module": "ES2022",
    "target": "ES2022"
