...
async function build_runtime ({ libs = lo.builtins(), bindings = lo.libraries(), embeds = [] }, verbose = false) {
  ...
  create_builtins([...libs, ...embeds], 'win')
  create_builtins([...libs, ...embeds], 'linux')
  create_builtins([...libs, ...embeds], 'mac')
...
const LO_HOME = getenv('LO_HOME') || `${getcwd()}/.lo`
...
async function build (args) {
  ...
  if (action === 'runtime') {
    ...
    if (args.length > 2 && args[2] === 'init') {
      ...
      if (!isFile(file_path)) {
        writeFile(file_path, encoder.encode(`
  const api = {
    noop: {
      parameters: [],
      result: 'void'
  ...
  `))
        ...
        console.log(`${AR}error${AD} binding ${name} already exists`)
      }
    } else {
