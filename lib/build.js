...
let link_args = '-s -O3 -fvisibility=hidden'
if (os === 'mac') {
  if (arch === 'arm64') {
    link_args = '-s -arch arm64 -w -O3 -fvisibility=hidden'
  } else {
    link_args = '-s -w -O3 -fvisibility=hidden'
  }
} else if (os === 'linux') {
  link_args = '-s -fno-exceptions -O3 -fvisibility=hidden'
}
...
const CFLAGS = (getenv('CFLAGS') || `-fPIC -std=c++20 -c -fomit-frame-pointer -fno-rtti -fno-exceptions -fvisibility=hidden -DV8_TYPED_ARRAY_MAX_SIZE_IN_HEAP=64 -DV8_ALLOCATION_FOLDING -DV8_SHORT_BUILTIN_CALLS`).split(' ')
