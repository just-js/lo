...
// https://copyprogramming.com/howto/what-are-the-differences-between-so-and-dylib-on-macos
  ...
  if (os === 'mac') {
    exec2([...LINK.split(' '), '-bundle_loader', '../../lo', ...LARGS, OPT, '-bundle', ...WARN, '-o', 
      ...
      ...(def.libs || []).map(l => `-l${l}`), '../../v8/libv8_monolith.a', `../../${TARGET}.o`],
      verbose)
  } else if (os === 'linux') {
...
const C = getenv('C') || (os === 'mac' ? 'gcc -arch arm64' : 'gcc')
const CC = getenv('CC') || 'g++'
const CFLAGS = (getenv('CFLAGS') || `${os === 'mac' ? '-arch arm64 ' : ''}-fPIC -std=c++17 -c -DV8_COMPRESS_POINTERS -DV8_TYPED_ARRAY_MAX_SIZE_IN_HEAP=0`).split(' ')
const LARGS = (getenv('LARGS') || (os === 'mac' ? '-arch arm64 -w -s' : '-s')).split(' ')
