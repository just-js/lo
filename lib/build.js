...
function bootstrap_app_dir (app_name, config) {
  const files = {
    [config.default.project_config]: () => generate_config(config.default),
    ['.gitignore']: () => `.lo
lib/**/*.cc
lib/**/*.o
lib/**/*.a
...
\`\`\`
  ...
  for (const k in files) {
    !is_file(k) && write_file(k, encoder.encode(files[k]()))
  }
}
async function init (args = ['']) {
  ...
  config.default.project_config = `${app_name}.config.js`

  bootstrap_app_dir(app_name, config)

  // add types if we can
  try {
    const { add_app_type_deps } = await import('lib/types.js')
    add_app_type_deps(config.default.project_config)
  } catch(e){
    console.log(`failed to add type deps\n${e?.toString()}`)
  }
}
...
const WARN = (getenv('LO_WARN') ||
  '-Werror -Wpedantic -Wall -Wextra -Wno-unused-parameter -Wno-template-id-cdtor').split(' ')
const cwd = getcwd()
...
const CFLAGS = (getenv('CFLAGS') || `-fPIC -std=c++17 -c -DV8_COMPRESS_POINTERS -DV8_TYPED_ARRAY_MAX_SIZE_IN_HEAP=0 -DV8_INTL_SUPPORT=1 -DENABLE_HUGEPAGE`).split(' ')
