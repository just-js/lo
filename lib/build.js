...
import { fileName, extName, join } from 'lib/path.js'
...
async function create_lo_home () {
  if (!isDir(LO_HOME)) {
    ...
    untar(inflate(bytes))
    unlink(file_name)
    assert(isDir(`${project}-${VERSION}`))
    rename(`${project}-${VERSION}`, '.lo')
    assert(isDir(LO_HOME))
  }
...
export { 
  build, build_runtime, create_binding, compile_bindings,
  config
}
