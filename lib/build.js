...
/*
    exec2([...LINK.split(' '), ...LARGS, ...opt.split(' '), '-shared', ...WARN, '-o', 
      `${def.name}.so`, `${def.name}.a`, 
//      ...(def.obj || []), 
//      ...(def.obj || []).filter(f => (extName(f) !== 'a' && f !== `${def.name}.o`)), 
      ...(def.libs || []).map(l => `-l${l}`), 
      ...(def.lib_paths || []).map(l => `-L${l}`)],
      verbose)
*/
...
//  runtimes.builder.bindings.push('kevents')
} else if (os === 'linux') {
//  runtimes.builder.bindings.push('epoll')
}
...
export { 
  build, build_runtime, create_binding, compile_bindings, runtimes, config 
}
