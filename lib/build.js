...
import {
  bindings, linkerScript, config, linkArgs, libPaths
} from 'lib/gen.js'
...
const {
  write_file, chdir, read_file, unlink, rename,
  S_IRWXU, S_IRWXG, S_IROTH, defaultWriteFlags
} = core

function exec2 (args, verbose = false) {
  if (verbose) console.log(args.join(' '))
  exec(args[0], args.slice(1), status)
  if (status[2]) console.error(`${args[0]} killed by signal ${status[2]}`)
  assert(status[0] === 0)
}
...
async function compile_bindings (lib, verbose = false, opt = OPT, shared = true, commands = []) {
  const cwd = getcwd()
  check_compilers()
/*
...
*/
  ...
  unlink(`${def.name}.o`)
  const cc_args = [...CXX.split(' '), ...CFLAGS, ...opt.split(' '), `-I${LO_HOME}`, `-I${LO_HOME}/v8`,
    ...include_paths.map(p => `-I${p}`), '-I.', `-I${LO_HOME}/v8/include`,
    `-I${lib_dir}`, ...WARN, '-o', `${def.name}.o`, `${def.name}.cc`]
  exec2(cc_args, verbose)
  commands.push({ command: cc_args.join(' '), directory: `${join(cwd, lib_dir)}`, file: `${join(join(cwd, lib_dir), def.name)}.cc` })
...
function create_builtins (libs = [], main = 'main.js', os, extra = []) {
  const cwd = getcwd()
  ...
  write_file(`${LO_HOME}/builtins_linux.S`,
    encoder.encode(`${Array.from(new Set([main, ...libs, ...extra])).map(verify_path).join('')}.section .note.GNU-stack,"",@progbits`))
  config.os = 'mac'
  write_file(`${LO_HOME}/builtins.S`,
    encoder.encode(Array.from(new Set([main, ...libs, ...extra])).map(verify_path).join('')))
  config.os = os
}

function create_header (libs = [], bindings = [], index = '', main = 'main.js', opts, snapshot = '') {
  const rx = /[./-]/g
  const main_name = main.replace(rx, '_')
  const snapshot_name = snapshot.replace(rx, '_')
...
${snapshot ? `extern char _binary_${snapshot_name}_start[];
extern char _binary_${snapshot_name}_end[];` : ''}

#endif

extern "C" {
  ${bindings.map(b => {
    if (b.constructor.name === 'String') return `extern void* _register_${b}();\nextern const intptr_t* _external_references_${b}();`
...
return `
  ...
  extern const intptr_t* _external_references_${name}();
#endif
`
    })
  }).join('\n  ')}
...
extern "C" const intptr_t* combined_binding_external_references() {
  ...
  if (combined.empty()) {
    ${bindings.map(b => {
      if (b.constructor.name === 'String') return `for (const intptr_t* p = _external_references_${b}(); *p; p++) combined.push_back(*p);`
      const name = Object.keys(b)[0]
      const platforms = b[name]
      return platforms.map(p => {
return `
#ifdef ${ifdefs[p]}
    for (const intptr_t* p = _external_references_${name}(); *p; p++) combined.push_back(*p);
#endif
`
      })
    }).join('\n    ')}
...
${snapshot ? `static v8::StartupData lo_snapshot_blob = { _binary_${snapshot_name}_start,
  (int)(_binary_${snapshot_name}_end - _binary_${snapshot_name}_start) };
static const v8::StartupData* lo_snapshot_data = &lo_snapshot_blob;` :
...
async function build_runtime ({ libs = lo.builtins(), bindings = lo.libraries(),
  ...
  v8_opts = {}, index = '', main = 'main.js', link_args = LARGS,
  // Optional: libs/embeds to keep in the FINAL, post-snapshot relink.
  // Defaults to [...libs, ...embeds] (today's behavior, unchanged) when
  // not given. Real use case: a runtime whose entry script fully
  // resolves+evaluates everything it needs during the snapshot build
  // pass (checked directly against V8's real serializer source,
  // 2026-08-27 - a Script's source string is never stripped during
  // serialization, only its line-ends/compiled-lazy-positions caches
  // are, so even a lazily-uncompiled function can still lazy-parse from
  // the blob's own embedded copy post-restore) never needs its embedded
  // .js/.d.ts source again after the blob exists - re-embedding it in
  // the final binary is pure redundancy. Set to [] (or a smaller list)
  // to drop what the FIRST pass still needs (to actually produce the
  // blob) from the SECOND, final relink. See V8-SNAPSHOT.md/PLAN.md
  // task 73 in the main tracking repo for the full reasoning.
  post_snapshot_embeds = null }, verbose = false) {
  const cwd = getcwd()
  ...
  if (os === 'linux' && MUSL) {
    // v8/libv8_monolith.a is a glibc build - these glibc-only symbols
    // (getrlimit64, backtrace, etc.) need the same shim object the
    // top-level Makefile links in via its own MUSL-gated BINDINGS entry
    // (see LO-MUSL.md). -U_LARGEFILE64_SOURCE cancels the -D this file
    // would otherwise pick up from CFLAGS, same reasoning as the
    // Makefile's own musl-glibc-compat.o rule: that define renames this
    // file's own *64 definitions out from under it.
    console.log(`${AY}compile${AD} musl-glibc-compat.c`)
    exec2([...CC.split(' '), '-fPIC', '-c', '-fomit-frame-pointer', '-fvisibility=hidden',
      '-D_LARGEFILE64_SOURCE', '-U_LARGEFILE64_SOURCE', '-O3',
      '-o', 'musl-glibc-compat.o', 'musl-glibc-compat.c'], verbose)
  }
  const commands = []
  console.log(`${AY}compile${AD} main.cc`)
  const main_cc_args = [...CXX.split(' '), `-DRUNTIME="${RUNTIME}"`, `-DVERSION="${VERSION}"`,
    ...CFLAGS, ...opt.split(' '), `-I${LO_HOME}`, `-I${LO_HOME}/v8`,
    '-I.', `-I${LO_HOME}/v8/include`, ...WARN, '-o', 'main.o', 'main.cc']
  exec2(main_cc_args, verbose)
  commands.push({ command: main_cc_args.join(' '), directory: cwd, file: join(cwd, 'main.cc') })
  console.log(`${AY}compile${AD} lo.cc`)
  const lo_cc_args = [...CXX.split(' '), `-DRUNTIME="${RUNTIME}"`, `-DVERSION="${VERSION}"`,
    ...CFLAGS, ...opt.split(' '), `-I${LO_HOME}`, `-I${LO_HOME}/v8`,
    '-I.', `-I${LO_HOME}/v8/include`, ...WARN, '-o', `${target}.o`, `lo.cc`]
  exec2(lo_cc_args, verbose)
  commands.push({ command: lo_cc_args.join(' '), directory: cwd, file: join(cwd, 'lo.cc') })
...
*/
//  static_libs = static_libs.concat(await Promise.all(platform_bindings.map(binding => compile_bindings(binding, verbose, opt, false))))
  for (const binding of platform_bindings) {
    static_libs = static_libs.concat(await compile_bindings(binding, verbose, opt, false, commands))
  }
...
//    ...static_libs, ...dynamic_libs, ...lib_paths].filter(v => v).flat(), verbose)
  exec2([...LINK.split(' '), ...link_args, ...link_type.split(' '), ...WARN, '-o',
    `${bin}`, `${target}.o`, 'main.o', 'builtins.o', 'v8/libv8_monolith.a',
    ...(os === 'linux' && MUSL ? ['musl-glibc-compat.o'] : []),
    ...static_libs, ...dynamic_libs, ...lib_paths].filter(v => v).flat(), verbose)
  assert(chdir(cwd) === 0)

  if (v8_opts.snapshot) {
    // second pass: run the binary just linked above in its own
    // build-snapshot mode (main.cc's --build-snapshot flag -> lo::
    // CreateSnapshot, see PLAN.md task 64) to produce a real V8 startup
    // snapshot blob of this runtime's own main script, then embed that
    // blob into the binary the same way main.js/lib/*.js source is
    // already embedded (.incbin, via create_builtins/create_header),
    // and re-link. Same two-stage shape V8's own mksnapshot uses for
    // V8's own default snapshot - build once, run the result to
    // produce an artifact, build/link again embedding that artifact.
    console.log(`${AY}build${AD} snapshot`)
    const snapshot_path = `${target}.snapshot.bin`
    // exec()/execvp needs a real path, not a bare command name relying
    // on PATH lookup (known lib/proc.js gotcha, LO-REFERENCE.md) - bin
    // is bare when cwd === LO_HOME, which execvp can't resolve.
    exec2([join(cwd, bin), '--build-snapshot', snapshot_path,
      ...(v8_opts.keep ? ['--keep'] : [])], verbose)
    if (v8_opts.snapshot_out) {
      write_file(v8_opts.snapshot_out, read_file(snapshot_path))
    }

    const final_embeds = post_snapshot_embeds === null ? [...libs, ...embeds] : post_snapshot_embeds
    console.log(`${AY}create${AD} builtins (with snapshot)`)
    create_builtins(final_embeds, main, config.os, [snapshot_path])
    console.log(`${AY}create${AD} main header (with snapshot)`)
    create_header(final_embeds, bindings, index, main, defaultOpts,
      snapshot_path)
    if (v8_opts.refs_out && is_file(join(LO_HOME, 'main.h'))) {
      write_file(v8_opts.refs_out, read_file(join(LO_HOME, 'main.h')))
    }

    assert(chdir(LO_HOME) === 0)
    console.log(`${AY}recompile${AD} builtins (with snapshot)`)
    if (os === 'linux') {
      exec2([...CC.split(' '), '-c', 'builtins_linux.S', '-o', 'builtins.o'], verbose)
    } else if (os !== 'win') {
      exec2([...CC.split(' '), '-c', 'builtins.S', '-o', 'builtins.o'], verbose)
    }
    console.log(`${AY}recompile${AD} main.cc (with snapshot)`)
    exec2(main_cc_args, verbose)
    commands.push({ command: main_cc_args.join(' '), directory: cwd, file: join(cwd, 'main.cc') })

    assert(chdir(cwd) === 0)
    console.log(`${AY}re-link runtime (with snapshot)${AD}`)
    exec2([...LINK.split(' '), ...link_args, ...link_type.split(' '), ...WARN, '-o',
      `${bin}`, `${target}.o`, 'main.o', 'builtins.o', 'v8/libv8_monolith.a',
      ...(os === 'linux' && MUSL ? ['musl-glibc-compat.o'] : []),
      ...static_libs, ...dynamic_libs, ...lib_paths].filter(v => v).flat(), verbose)
  ...
  write_file('compile_commands.json', encoder.encode(JSON.stringify(commands, null, '  ')))
}
...
if (lo.core.os === 'linux') link_type += ' -fuse-ld=lld -static-libgcc'
if (lo.core.os === 'mac') {
  link_type += ' -w -framework CoreFoundation'
  bindings.push('mach')
}
...
let OPT = getenv('LO_OPT') || '-O3'
const WARN = (getenv('LO_WARN') ||
  '-Werror -Wpedantic -Wall -Wextra -Wno-unused-parameter -Wno-error=unknown-warning-option').split(' ')
const cwd = getcwd()
...
if (os === 'mac') {
  if (arch === 'arm64') {
    link_args = '-s -arch arm64 -w -O3'
    OPT = `${OPT} -march=native -mtune=native`
  } else {
    link_args = '-s -arch x86_64 -w -O3'
  }
} else if (os === 'linux') {
  link_args = '-s -fno-exceptions -O3'
  OPT = `${OPT} -march=native -mtune=native`
}
...
const CFLAGS = (getenv('CFLAGS') || `-fPIC -std=c++20 -c -fomit-frame-pointer -fno-rtti -fno-exceptions -DV8_TYPED_ARRAY_MAX_SIZE_IN_HEAP=64 -DV8_ALLOCATION_FOLDING -DV8_SHORT_BUILTIN_CALLS -DV8_COMPRESS_POINTERS${MUSL ? ' -D_LARGEFILE64_SOURCE' : ''}`).split(' ')
...
if (os === 'mac') {
  ...
  const max_path = lo.ptr(new Uint32Array([1024]))
  const path_name = lo.ptr(new Uint8Array(1024))
  assert(mach.get_executable_path(path_name.ptr, max_path.ptr) === 0)
  LO_PATH = lo.latin1Decode(path_name.ptr, max_path[0])
} else if (os === 'linux') {
  const path_name = lo.ptr(new Uint8Array(1024))
  const len = core.readlink('/proc/self/exe', path_name.ptr, 1024)
  assert(len > 0)
