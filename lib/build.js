...
import {
  bindings, bindingsAbi, linkerScript, config, linkArgs, libPaths
} from 'lib/gen.js'
...
*/
  ...
  // Which codegen target lib/gen.js emits for this binding -- an env var
  // override always wins (whichever's easiest to flip per-invocation,
  // e.g. for A/B benchmarking), else the binding's own api.js opts in via
  // `target = 'abi'`, else today's status quo (V8-specific codegen) so
  // every existing binding is unaffected. See doc/WORK.E.1.md's "Result"
  // section / lib/gen.js's bindingsAbi() for what the 'abi' target
  // actually supports (deliberately narrower than 'v8' today).
  const target = getenv('LO_GEN_TARGET') || def.target || 'v8'
  const src = target === 'abi' ? bindingsAbi(def) : bindings(def)
  ...
  if (is_file(build_path)) {
    ...
    await build(CC, CXX, CFLAGS)
  } else {
    console.log(`${AY}change dir to ${AD} ${lib_dir}`)
    assert(chdir(lib_dir) === 0)
  }
  // lo_abi_v8.cc (every lo_abi.h-targeted binding's shared V8 backend)
  // used to be recompiled and linked into *each* abi-target binding's
  // own .a/.so here -- real, measurable duplication (one full recompile
  // per binding, plus a copy of the dispatch tables baked into every
  // .so) once more than one or two such bindings existed. It's compiled
  // exactly once now, into the runtime binary itself (see
  // build_runtime's own lo_abi_v8.cc step), and exported the same way
  // lo.cc's SET_VALUE/SET_FAST_METHOD/SET_MODULE already are
  // (-rdynamic, already the default link_type for every runtime config
  // that supports dynamic binding loading at all) -- a dynamically
  // loaded binding's lo_register_functions/lo_exports_set_* calls
  // resolve against the running process at dlopen time, same mechanism,
  // nothing binding-specific to compile here anymore. This does mean a
  // binding built this way needs a `lo` binary that actually includes
  // lo_abi_v8.cc (i.e. rebuilt after this change) -- an older binary
  // won't have these symbols exported.
...
async function build_runtime ({ libs = lo.builtins(), bindings = lo.libraries(),
  ...
  // lo_abi_v8.cc, the shared V8 backend every lo_abi.h-targeted binding
  // needs (lo_register_functions/lo_exports_set_*/...) -- compiled once
  // into the runtime itself now, not once per binding (see
  // compile_bindings' own comment on why). Always compiled, same as
  // lo.cc itself, even for a runtime config that never loads an abi
  // binding -- cheap, and simpler than threading a flag through every
  // runtime/*.config.js to opt in.
  console.log(`${AY}compile${AD} lo_abi_v8.cc`)
  const lo_abi_v8_cc_args = [...CXX.split(' '), ...CFLAGS, ...opt.split(' '),
    `-I${LO_HOME}`, `-I${LO_HOME}/v8`, `-I${LO_HOME}/v8/include`,
    '-o', 'lo_abi_v8.o', 'lo_abi_v8.cc']
  exec2(lo_abi_v8_cc_args, verbose)
  commands.push({ command: lo_abi_v8_cc_args.join(' '), directory: cwd, file: join(cwd, 'lo_abi_v8.cc') })
  // lo_ffi.cc, the generic dlopen'd-FFI-call machinery lib/core's
  // bind_fastcall/bind_slowcall/fastcall need -- unlike lo_abi_v8.cc
  // above, compiled and linked *only* when this runtime config actually
  // includes a binding that needs it ('core'/'core2' today). Real
  // difference from that "always compile" call: runtime/zero.config.js
  // deliberately builds with `bindings: []` (no core at all), and this
  // FFI machinery -- real CTypeInfo/CFunctionInfo/CFunction construction
  // -- has genuine weight not worth carrying into a build that can never
  // reach it. See lo.h's own comment on why this lives in its own
  // translation unit rather than lo.cc.
  const needs_ffi = platform_bindings.includes('core') || platform_bindings.includes('core2')
  if (needs_ffi) {
    console.log(`${AY}compile${AD} lo_ffi.cc`)
    const lo_ffi_cc_args = [...CXX.split(' '), ...CFLAGS, ...opt.split(' '),
      `-I${LO_HOME}`, `-I${LO_HOME}/v8`, `-I${LO_HOME}/v8/include`,
      '-o', 'lo_ffi.o', 'lo_ffi.cc']
    exec2(lo_ffi_cc_args, verbose)
    commands.push({ command: lo_ffi_cc_args.join(' '), directory: cwd, file: join(cwd, 'lo_ffi.cc') })
  }
...
//    ...static_libs, ...dynamic_libs, ...lib_paths].filter(v => v).flat(), verbose)
  exec2([...LINK.split(' '), ...link_args, ...link_type.split(' '), ...WARN, '-o',
    `${bin}`, `${target}.o`, 'main.o', 'builtins.o', 'lo_abi_v8.o', 'v8/libv8_monolith.a',
    ...(needs_ffi ? ['lo_ffi.o'] : []),
