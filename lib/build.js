...
*/
  ...
  if (target === 'abi') {
    // Every lo_abi.h-targeted binding needs the same shared V8 backend
    // (lo_abi_v8.cc) linked in -- compiled here, once, generically,
    // rather than requiring each binding to hand-write its own build.js
    // just to do this (real duplication this replaced, see
    // lib/foo_abi/build.js's own history/removal). Always recompiled
    // fresh, never skipped-if-exists -- a stale cached lo_abi_v8.o
    // silently ignoring real edits to lo_abi_v8.cc was a real bug hit
    // directly during this prototype's development.
    console.log(`${AY}compile${AD} lo_abi_v8.cc ${AY}with${AG} ${CXX}${AD}`)
    unlink('lo_abi_v8.o')
    exec2([...CXX.split(' '), ...CFLAGS, ...opt.split(' '),
      `-I${LO_HOME}`, `-I${LO_HOME}/v8`, `-I${LO_HOME}/v8/include`,
      '-o', 'lo_abi_v8.o', `${LO_HOME}/lo_abi_v8.cc`], verbose)
    obj = obj.concat(['lo_abi_v8.o'])
  }
