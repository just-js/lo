...
function getAbiType (t) {
  ...
  // Same as 'pointer'/'buffer', not a distinct marshaling case: the
  // active getFastParameterCast/getSlowParameterCast code for 'u32array'
  // (this file, above) already just reinterpret_casts the raw JS number
  // straight to a pointer type -- there's no separate array-aware
  // unwrapping in the real, active codegen (the Uint32Array-unwrap
  // version is commented out, same as 'buffer's). See lib/curl/api.js's
  // easy_getinfo for a real caller.
  if (t === 'u32array') return 'LO_U32ARRAY'
  throw new Error(`lib/gen.js (abi target): type '${t}' isn't supported by lo_abi_v8.cc's dispatch yet (no f32/f64/function -- see doc/WORK.E.1.md)`)
}
...
function getAbiFastCType (t) {
  ...
  if (t === 'usz' || t === 'pointer' || t === 'buffer' || t === 'u32array') return 'uint64_t'
  if (t === 'void') return 'void'
  return null
}
...
function isAbiFastEligible (fn) {
  ...
  // Every real `override` co-occurs with a 'string' parameter today,
  // which already excludes it below (getAbiFastCType('string') is
  // null) -- but that's a coincidence of current bindings, not a
  // guarantee. genAbiFastWrapper doesn't know about `overrides` at all
  // (it isn't a slow-tier-only concept the way it's implemented in
  // lo_abi_v8.cc's DispatchGeneral), so a hypothetical override without
  // an accompanying string would otherwise silently generate a fast
  // wrapper with the wrong parameter count. Explicit, not relied on.
  if (fn.override) return false
...
// `override` support for the abi target -- surveyed all 13 real call
// sites across every binding directly (not guessed) before writing
// this: every single one is one of exactly two shapes, both trailing-
// only (no ordinary parameter ever follows an overridden one in any
// real binding):
//   - `{ param: N, fastfield: '->length', slowfield: '.length()' }` --
//     "derive this parameter from strlen() of the already-'string'-typed
//     parameter N" (12/13 uses -- e.g. `write_string(fd, str)` calling
//     the real 3-arg `write(fd, buf, len)`).
//   - a literal `0` (1/13 uses -- `net`'s `send_string`, a trailing
//     `flags` parameter hardcoded to 0).
// Returns null if `fn` has no `override`, else an array with one entry
// per parameter: -1 (no override), the string 'zero' (literal-0
// override), or a number (strlen-of-parameter-N override). Throws for
// any shape beyond these two, or a non-trailing override -- the
// V8-specific codegen's `override` also allows arbitrary Number/String
// literals and arbitrary field-access expressions, but nothing real
// uses either, so there's nothing to generalize to yet.
function validateAbiOverride (name, fname, fn) {
  if (!fn.override) return null
  const nparams = fn.parameters.length
  const marks = new Array(nparams).fill(-1)
  let sawOverride = false
  for (let i = 0; i < nparams; i++) {
    const o = fn.override[i]
    if (o === undefined) {
      if (sawOverride) {
        throw new Error(`lib/gen.js (abi target): '${name}.${fname}' has an ordinary parameter after an overridden one (index ${i}) -- the abi target only supports trailing overrides`)
      }
      continue
    }
    sawOverride = true
    ...
    if (o && typeof o === 'object' && o.fastfield === '->length' && o.slowfield === '.length()' && typeof o.param === 'number') {
      if (fn.parameters[o.param] !== 'string') {
        throw new Error(`lib/gen.js (abi target): '${name}.${fname}''s override at index ${i} references parameter ${o.param}, which isn't 'string'`)
      }
      marks[i] = o.param
      continue
    }
    throw new Error(`lib/gen.js (abi target): '${name}.${fname}' uses an 'override' shape the abi target doesn't support yet (only a trailing length-of-a-preceding-string, or a literal 0, is handled)`)
  }
  return marks
}

function bindingsAbi ({ api, name, preamble, constants = {}, structs = [], includes = [], externs = [] }) {
  // 'fastcall' (lib/core's own structs list) is the one struct anywhere
  // in this codebase declared in lo.h rather than a real system header
  // -- same lo.h-visibility gap as the no_abi functions above, but
  // structs has no per-entry way to say so (it's a plain array of
  // names, unlike api's per-function objects), so it's just named here.
  const abiStructs = structs.filter(s => s !== 'fastcall')
  const constantRows = Object.keys(constants).map(n => initConstantAbi(n, constants[n])).join('') +
    abiStructs.map(initStructAbi).join('')

  // `declare_only` (see lib/heap/api.js's `snapshot`) means "this
  // function's whole implementation is hand-written in the preamble
  // because it's inherently engine-specific" -- no `parameters`/`result`
  // even exist on the definition to run through getAbiType(). Unlike
  // `override`/a dereference `casts` entry (real, fixable gaps in this
  // target's dispatch, rejected above/below), a `declare_only` function
  // can never become portable -- it exists specifically to call engine
  // APIs (heap.snapshot: v8::HeapProfiler/v8::OutputStream) with no
  // lo_type_t equivalent, by design, not because bindingsAbi() hasn't
  // caught up yet. So it's excluded from the abi target's exports
  // entirely, cleanly, rather than thrown on -- same as it simply
  // wouldn't exist if this binding only ever targeted the abi backend.
  //
  // `no_abi` (see lib/core/api.js's `fastcall`/`isolate_*`/`callback`)
  // is the same idea for a function that *does* have a normal
  // parameters/result shape but whose real symbol (`lo_fastcall`,
  // `lo_create_isolate`, ...) is declared only in lo.h, which
  // bindingsAbi()'s generated files deliberately never #include (that's
  // what keeps them from pulling in v8:: transitively) -- genuinely not
  // resolvable here, not a fixable gap.
  const fNames = Object.keys(api).filter(fname => !api[fname].declare_only && !api[fname].no_abi)
  for (const fname of fNames) {
    const fn = api[fname]
    fn.name = fn.name || fname
    // `casts`/`rpointer` need no special handling for most real uses --
    // both codegens' dispatch here goes through a canonical, type-erased
    // function pointer cast (see lo_abi_v8.cc's own comments), so a pure
    // bit-identical reinterpretation cast (`(CURLINFO)`, `(pthread_t)`,
    // `(const unsigned char*)`, ...) or a return-type-only override
    // (`rpointer`) transmits the exact same bits either way -- verified
    // against a real caller, lib/curl's `easy_getinfo` (`casts: [,
    // '(CURLINFO)']`), which runs correctly under the abi target with no
    // codegen change at all. A *dereference* cast (`lib/raylib`'s
    // `'*(Color*)'`, `lib/wireguard`'s bare `'*'`) is a genuinely
    // different, unsupported case, though -- it means the real C
    // function takes a small struct *by value* (e.g. `ClearBackground
    // (Color color)`), and the canonical uint64_t dispatch would pass
    // the raw pointer's bit pattern as if it *were* the struct's bytes,
    // silently wrong rather than a compile error. No `lo_type_t` for
    // "struct passed by value" exists yet (same underlying gap as
    // f32/f64's register-class work, not done) -- reject clearly instead
    // of emitting code that runs but produces garbage.
    if ((fn.casts || []).some(c => typeof c === 'string' && c.trim().startsWith('*'))) {
      throw new Error(`lib/gen.js (abi target): '${name}.${fname}' uses a dereference cast (by-value struct argument), which the abi target doesn't support yet`)
    }
  ...
  const overrides = {}
  for (const fname of fNames) overrides[fname] = validateAbiOverride(name, fname, api[fname])
  ...
  const overrideArrays = fNames.filter(fname => overrides[fname]).map(fname => {
    const marks = overrides[fname].map(m => m === -1 ? 'LO_NO_OVERRIDE' : m === 'zero' ? 'LO_OVERRIDE_LITERAL_ZERO' : String(m))
    return `static const uint8_t ${fname}_overrides[] = { ${marks.join(', ')} };\n`
  }).join('')
  ...
  const descRows = fNames.map((fname, i) => {
    ...
    const overridesRef = overrides[fname] ? `${fname}_overrides` : 'nullptr'
    return `  { "${fname}", ${getAbiType(fn.result)}, ${paramsRef}, ${fn.parameters.length}, (void*)${fn.name}, ${fastWrappers[i].ref}, ${overridesRef}, 0 },\n`
  }).join('')
...
${paramArrays}${overrideArrays}${fastWrapperDecls}${fNames.length ? `static const lo_fn_desc_t ${name}_fns[] = {
${descRows}};
` : ''}
LO_REGISTER(${name}) {
${fNames.length ? `  lo_status_t status = lo_register_functions(engine, exports, ${name}_fns,
