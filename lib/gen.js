...
async function bindings (importPath) {
  ...
  function getFunction (n) {
    const definition = api[n]
    const { parameters, pointers, result, name, rpointer, nofast, casts = []} = definition
    let src = `
void ${name}Slow(const FunctionCallbackInfo<Value> &args) {\n`
    ...
    if (result === 'void') {
      ...
      src += `  ${rpointer || getType(result)} rc = ${n}(${parameters.map((p, i) => `${getCast(i)}v${i}`).join(', ')});\n`
    }
    function getCast (i) {
    ...
    src += `
${needsUnwrap(definition.result) ? 'void' : getType(result, true)} ${name}Fast(void* p${(parameters.length || needsUnwrap(definition.result)) ? ', ' : ''}${getParams(definition)}) {
${parameters.map((p, i) => getFastParameterCast(p, i, pointers)).join('\n')}`
    if (result === 'void') {
      src += `\n  ${n}(${parameters.map((p, i) => `${getCast(i)}v${i}`).join(', ')});`
    } else if (needsUnwrap(result)) {
      src += `\n  ${rpointer || getType(result)} r = ${n}(${parameters.map((p, i) => `${getCast(i)}v${i}`).join(', ')});\n`
      src += `  ((${rpointer || getType(result)}*)p_ret->data)[0] = r;\n`
    } else {
      src += `\n  return ${n}(${parameters.map((p, i) => `${getCast(i)}v${i}`).join(', ')});`
    }
    src += '\n}'
    return src
  }
...
library: ${obj.filter(o => o !== name).map(o => `${o} `).join('')}## compile shared library and dependencies
	...
	$(CC) -flto -g -O3 -shared -flto -pthread -m64 -Wl,--start-group ${obj.filter(o => o !== name).map(o => `${o} `).join('')}$(MODULE).o -Wl,--end-group -Wl,-soname=$(MODULE).so ${libs.map(l => `-l${l} `).join('')}-o $(MODULE).so
