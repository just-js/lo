...
async function bindings (importPath) {
  ...
  function initFunction (n) {
    ...
    if (nofast) {
      return `  SET_METHOD(isolate, module, "${n}", ${name}Slow);\n`
    }
  ...
  v8::CFunction* pF${name} = new v8::CFunction((const void*)&${n}Fast, info${n});
  ...
  function getFunction (n) {
    const definition = api[n]
    const { parameters, pointers, result, name = n, rpointer, nofast, casts = []} = definition
    let src = `
void ${n}Slow(const FunctionCallbackInfo<Value> &args) {\n`
    if ((result !== 'void' && result !== 'pointer') || parameters.includes('pointer') || parameters.includes('string')) {
    ...
    if (result === 'void') {
      src += `  ${name}(${parameters.map((p, i) => `v${i}`).join(', ')});\n`
    } else {
...
${parameters.map((p, i) => getFastParameterCast(p, i, pointers)).join('\n')}`
    if (result === 'void') {
      src += `\n  ${name}(${parameters.map((p, i) => `${getCast(i)}v${i}`).join(', ')});`
      ...
      src += `\n  return ${name}(${parameters.map((p, i) => `${getCast(i)}v${i}`).join(', ')});`
    }
    src += '\n}'
    return src
  }
