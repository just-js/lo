...
function bindings ({ api, includes = [], name, preamble, constants, structs = [] }) {
  const fNames = []
  ...
  function initConstant(n) {
    ...
    if (type ==='i64') {
      return `  SET_VALUE(isolate, module, "${n}", BigInt::New(isolate, (int64_t)${type}));\n`
    }
    if (isNumeric(type)) {
...
function wrapper ({ api, includes, name, libs = [], make, obj = [], deps = [] }) {
...
async function linkArgs (deps = [], opts = defaultOpts) {
  ...
  const link_args = Array.from(new Set(imports.map(b => b.libs || []).flat()))
...
async function gen (args) {
  ...
  if (args[0] === '--builtins') {
    ...
    source = wrapper(await import(args[1]))
  } else {
...
export { bindings, linkerScript, headerFile, wrapper, makeFile, linkArgs, config, gen }
