...
function bindings ({ api, includes = [], name, preamble, constants, structs = [], linux, mac, externs = [] }) {
  ...
  // one intptr_t per SnapshotCreator external_references entry - always
  // the plain slow FunctionCallback (registered either as the SET_METHOD
  // target directly, or as SET_FAST_METHOD's own slow-path fallback), plus,
  // for genuinely fast-registered functions, the same 3-address-per-site
  // shape lo.cc's own hand-written lo_external_references[] needs (the
  // CFunction object, its GetAddress(), and its GetTypeInfo() - V8 needs
  // an exact address match for all three, confirmed against Node's own
  // ExternalReferenceRegistry::Register(const v8::CFunction&), see the
  // comment above lo_external_references[] in lo.cc for the full history).
  function externalRef (n, api) {
    const definition = api[n]
    if (definition.nofast || definition.slow) {
      return `  (intptr_t)&${n}Slow,\n`
    }
    return `  (intptr_t)&${n}Slow,\n  (intptr_t)&pF${n}, (intptr_t)pF${n}.GetAddress(),\n    (intptr_t)pF${n}.GetTypeInfo(),\n`
  }
...
static const intptr_t ${name}_external_references[] = {
${fNames.map(n => externalRef(n, api)).join('')}
#ifdef __linux__
${linuxfNames.map(n => externalRef(n, linux.api)).join('')}
#endif
#ifdef __MACH__
${macfNames.map(n => externalRef(n, mac.api)).join('')}
#endif
  0
};
...
extern "C"  {
  ...
  DLL_PUBLIC const intptr_t* _external_references_${name}() {
    return ${config.runtime}::${name}::${name}_external_references;
  }
}
`
}
