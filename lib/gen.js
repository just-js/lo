...
// no constants/structs (lo_exports_set_*/struct support isn't implemented
// in lo_abi_v8.cc, and lo_abi.h has no struct equivalent by design).
// Throws clearly at generation time for any of these rather than
// emitting code that won't compile or would misbehave -- widen this
// alongside lo_abi_v8.cc gaining the matching dispatch support, not
// ahead of it.
...
function bindingsAbi ({ api, name, preamble, constants = {}, structs = [] }) {
  if (Object.keys(constants).length) {
    throw new Error(`lib/gen.js (abi target): '${name}' declares constants -- lo_abi_v8.cc doesn't implement lo_exports_set_* yet (see its own top comment)`)
  }
  if (structs.length) {
    throw new Error(`lib/gen.js (abi target): '${name}' declares structs -- lo_abi.h has no struct equivalent (deliberately out of scope, see lo_abi.h's design-principles comment)`)
  }
  ...
  const descRows = fNames.map(fname => {
    const fn = api[fname]
    const paramsRef = fn.parameters.length ? `${fname}_params` : 'nullptr'
    return `  { "${fname}", ${getAbiType(fn.result)}, ${paramsRef}, ${fn.parameters.length}, (void*)${fn.name}, 0 },\n`
  }).join('')
...
${paramArrays}static const lo_fn_desc_t ${name}_fns[] = {
${descRows}};

LO_REGISTER(${name}) {
  return lo_register_functions(engine, exports, ${name}_fns,
