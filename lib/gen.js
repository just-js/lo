...
${make || ''}
$(BINDING).so: ${obj.filter(o => o !== name).map(o => `${o} `).join('')}## compile shared library and dependencies
...
ifeq ($(os),mac)
#	$(CC) -v -fPIC $(LARGS) $(OPT) -dynamiclib $(WARN) -o $(BINDING).dylib ${obj.filter(o => o !== name).map(o => `${o} `).join('')}$(BINDING).o ${libs.map(l => `-l${l} `).join('')} $(${HOME})/v8/libv8_monolith.a $(${HOME})/${config.runtime}.o
else
...
export { bindings, linkerScript, headerFile, makeFile, config }
