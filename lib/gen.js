...
ifeq ($(os),mac)
	$(CC) -v -fPIC $(LARGS) $(OPT) -dynamiclib $(WARN) -o $(BINDING).dylib ${obj.filter(o => o !== name).map(o => `${o} `).join('')}$(BINDING).o ${libs.map(l => `-l${l} `).join('')} -L$(${HOME}) deps/libv8_monolith.a ${config.runtime}.o
else
