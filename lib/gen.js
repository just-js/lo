...
async function makeFile (importPath) {
  const { api, includes, name, libs = [], make, obj = [], deps = [], cflags = '-march=native -mtune=native -Wall -Wextra -Wno-unused-parameter' } = await import(importPath)
  return `
C=gcc
CC=g++
MODULE=${name}
...
library: ${obj.filter(o => o !== name).map(o => `${o} `).join('')}## compile shared library and dependencies
	$(CC) -c -flto -g -O3 -fPIC -std=c++17 -DV8_NO_COMPRESS_POINTERS -DV8_TYPED_ARRAY_MAX_SIZE_IN_HEAP=0 -I. -I$(SPIN_HOME) -I$(SPIN_HOME)/deps/v8/include ${cflags} -o $(MODULE).o $(MODULE).cc
	ar crsT $(MODULE).a $(MODULE).o ${obj.filter(o => o !== name).map(o => `${o} `).join('')}
