...
/*
  ...
  for (const name of Object.keys(api)) {
    const fn = api[name]
    if (!fn.platform || fn.platform.includes(config.os)) {
      if (!fn.arch || fn.arch.includes(config.arch)) {
        fn.pointers = fn.pointers || []
        fn.name = fn.name || name
        fNames.push(name)
      }
...
function makeFile ({ api, includes, name, libs = [], make, obj = [], deps = [] }) {
...
$(BINDING).a: ${obj.filter(o => o !== name).map(o => `${o} `).join('')}## compile static library and dependencies
	$(CXX) -fPIC $(CCARGS) $(OPT) -I$(${HOME}) -I$(${HOME})/v8 -I$(${HOME})/v8/include $(WARN) -o $(BINDING).o $(BINDING).cc
	ar crsT $(BINDING).a $(BINDING).o ${obj.filter(o => o !== name).map(o => `${o} `).join('')}

$(BINDING).so: $(BINDING).a ## compile shared library
ifeq ($(os),mac)
	$(CXX) -fPIC $(LARGS) $(OPT) -dynamiclib $(WARN) -o $(BINDING).dylib ${obj.filter(o => o !== name).map(o => `${o} `).join('')}$(BINDING).o ${libs.map(l => `-l${l} `).join('')} $(${HOME})/v8/libv8_monolith.a $(${HOME})/${config.runtime}.o
else
	$(CXX) $(LARGS) $(OPT) -shared $(WARN) -o $(BINDING).so ${obj.filter(o => o !== name).map(o => `${o} `).join('')}$(BINDING).o ${libs.map(l => `-l${l} `).join('')}
endif
...
//    source += await linkerScript('main.js')
    ...
    // todo: this is obsolete now we are using lo to build bindings rather than make
    source = makeFile(await import(args[1]))
  } else {
...
export { 
  bindings, linkerScript, headerFile, makeFile, linkArgs, libPaths, config, gen
}
