...
function make_args (args) {
  ...
  if (!args.length) return { args: ptr(new Uint8Array(0)) }
...
//    this.penv = make_args(venv)
  ...
  run () {
    ...
    if (pid === 0) {
      ...
      const { exe_path, pargs, venv } = this
      // we are in the child process
      if (this.cwd) chdir(this.cwd)
      if (venv) {
        for (let i = 0; i < venv.length; i++) {
          const parts = venv[i].split('=')
          setenv(parts[0], parts.slice(1).join(''))
        }
      }
      execvp(exe_path, pargs.args.ptr)
      lo.exit(1)
      return -1
    } else if (pid < 0) {
      this.error = lo.errno
  ...
  waitfor () {
    const { pid } = this
    const rc = waitpid(pid, stat.ptr, 0)
    if (rc > 0) {
      assert(rc === pid)
      this.status = stat[0]
    }
  ...
  poll () {
    const { pid } = this
    this.#stats()
    const rc = waitpid(pid, stat.ptr, WNOHANG)
    if (rc > 0) {
      assert(rc === pid)
      this.status = stat[0]
      return 1
  ...
  readout (buf, len = buf.length) {
    return read(this.stdout[READABLE], buf.ptr, len)
  }

  readerr (buf, len = buf.length) {
    return read(this.stderr[READABLE], buf.ptr, len)
  }

  writein (buf, len = buf.length) {
    return write(this.stdin[WRITABLE], buf.ptr, len)
  }
...
const stat = ptr(new Int32Array(2))
const READABLE = 0
const WRITABLE = 1
