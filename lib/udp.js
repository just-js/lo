...
class MsgHdr {
  constructor () {
    this.size = 56
    this.raw = ptr(new Uint8Array(56))
    this.view = new DataView(this.raw.buffer)
  }
...
class Node {
  ...
  bind (address, on_readable, port = 0, on_error = noop) {
    ...
    assert(bind(fd, src.ptr, src.length) === 0)
  ...
  connect (address, port) {
    return connect(this.fd, sockaddr_in(address, port).ptr, SOCKADDR_LEN)
  }
  ...
  recv (u8, flags = 0) {
    ...
    return recvmsg(this.fd, this.msghdr.raw.ptr, flags)
  }

  send (u8, size = u8.length, flags = 0) {
    this.iov.payload = u8
    this.iov.length = size
    return sendmsg(this.fd, this.msghdr.raw.ptr, flags)
  }

  sendto (u8, addr, size = u8.length, flags = 0) {
    this.iov.payload = u8
    this.iov.length = size
    return sendmsg(this.fd, addr.raw.ptr, flags)
  }
  ...
  get port () {
    ...
    net.get_sockname(this.fd, src.ptr)
