...
async function wrap_mem_usage () {
  if (globalThis.Deno) {
    if (Deno.build.os === 'linux') {
      ...
      const mem = () => Math.floor(usage().rss / 1024)
      return { mem, cputime: () => [] }
    }
    return { mem: () => 0, cputime: () => [] }
  }
  if (globalThis.Bun) {
...
class Bench {
  ...
  start (name = 'bench') {
    ...
    const rate = Math.floor(count / (elapsed / 1000))
    const nanos = Math.ceil((1000000000 / rate) * 1000) / 1000
    const rss = mem()
    ...
    if (this.#display) {
      console.log(`${AM}${this.#name.trim().padEnd(this.#name_width, ' ')}${AD} ${AY}time${AD} ${pad(Math.floor(elapsed), 6)} ${AY}rate${AD} ${pad(rate, 12)} ${AG}ns/iter${AD} ${pad(nanos, 8, 3)} ${AG}rss${AD} ${pad(rss, 8)} ${AY}usr${AD} ${usr.toString().padStart(3, ' ')} ${AY}sys${AD}  ${sys.toString().padStart(3, ' ')} ${AY}tot${AD} ${(usr + sys).toString().padStart(3, ' ')}`)
    }
