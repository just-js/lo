...
void SlowCallbackV(const FunctionCallbackInfo<Value> &args) {
  struct fastcall* state = static_cast<struct fastcall*>(args.Data().As<v8::External>()->Value(v8::kExternalPointerTypeTagDefault));  
  lo_fastcall(state);
}
*/
...
void bind_fastcallSlow(const FunctionCallbackInfo<Value> &args) {
  ...
  if (state->result == lo::FastTypes::i32) {
    funcTemplate = FunctionTemplate::New(
      isolate,
      make_stub(state, reinterpret_cast<void*>(&SlowCallbackI32)), 
      data, 
      Local<v8::Signature>(), 
      0, 
    ...
    funcTemplate = FunctionTemplate::New(
      isolate,
      make_stub(state, reinterpret_cast<void*>(&SlowCallbackV)), 
      data, 
      Local<v8::Signature>(), 
      0, 
      v8::ConstructorBehavior::kThrow,
