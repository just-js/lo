...
// todo: user 32 bit and 16 bit ops where possible and measure the difference
const { mprotect, memcpy, mmap, munmap } = system
...
function compile (code) {
  // todo: handle errors and ensure we cannot have writable/executable memory
...
class Assembler {
  ...
  mov (src, dest, soff = null, doff = null) {
    if (soff === null && doff === null) {
      ...
      if (dest === rdi) {
        this.#codes.push([0x48, 0xc7, 0xc7, src & 0xff, (src >> 8) * 0xff, (src >> 16) & 0xff, (src >> 24) & 0xff])
        return
      }
      ...
      if (soff) {
        if (doff) {
          ...
          if (src === rsp && dest === rsp) {
            if (soff >= 0x80) {
              this.#codes.push([0x48, 0x8b, 0x84, 0x24, soff, 0x00, 0x00, 0x00])
              //this.#codes.push([0x48, 0x8b, 0x83, 0x24, soff, 0x00, 0x00, 0x00])
            } else {
              this.#codes.push([0x48, 0x8b, 0x44, 0x24, soff])
              //this.#codes.push([0x48, 0x8b, 0x43, 0x24, soff])
            }
  ...
  codes () {
    return this.#codes.map(c => c.length ? c.map(cc => cc.toString(16)) : c.toString(16)).join('\n')
  }
