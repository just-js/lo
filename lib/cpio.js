...
const AG = "\u001b[92m" // ANSI Green
const AY = "\u001b[93m" // ANSI Yellow
const AM = "\u001b[95m" // ANSI Magenta
const AC = "\u001b[96m" // ANSI Cyan
...
if (globalThis.lo) {
  ...
  constants.S_IFLNK = lo.core.S_IFLNK
  ...
  constants.S_IFLNK = fs.constants.S_IFLNK
...
function build_cpio (entries) {
  ...
  function get_type (entry) {
    ...
    if (type === 'sym') return S_IFLNK
    return S_IFREG
  }

  function get_perms (entry) {
    const { type, exe } = entry
    if (type === 'reg' || type ==='sym') {
  ...
  function get_size (entry) {
    ...
    if (type === 'reg') {
      if (entry.source) {
        ...
        throw new Error(`no 'buffer' or 'source' field provider for entry`)
      }
    }
    if (type === 'sym') {
  ...
  function create_entry (entry) {
    ...
    const size = entry.size || hex(get_size(entry), 8)
    ...
    align()
    if (entry.type === 'sym') {
      payload = encoder.encode(string(entry.source))
...
function col (type, perms) {
  ...
  if (type === 'sym') return AM
  if (type === 'sck') return AG
