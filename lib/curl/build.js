...
async function build () {
  ...
  const SYSROOT = getenv('SYSROOT')
  const sysroot_flag = SYSROOT ? ` --sysroot=${SYSROOT}` : ''
...
//    assert(exec('make', ['clean'])[0] === 0)
    exec_env('./configure', curl_opts.split(' '), [['CFLAGS', `-fPIC -O3 -march=native -mtune=native -DOPENSSL_NO_ENGINE${sysroot_flag}`]])
    assert(exec_env('make', ['-j', '4'], [['CFLAGS', `-fPIC -O3 -march=native -mtune=native -DOPENSSL_NO_ENGINE${sysroot_flag}`]])[0] === 0)
    assert(chdir('../../') === 0)
  }
}
