import { mkDirAll, rmDirAll, isDir } from 'lib/fs.js'
import { baseName } from 'lib/path.js'

const { utf8Decode, ptr, assert } = lo
const {
  write, close, mkdir, open, O_TRUNC, O_CREAT, O_WRONLY, NAME_MAX, strnlen, O_RDWR
} = lo.core
...
function untar (u8, buf_size = u8.length) {
  ...
  while (size) {
    ...
    let fileName = utf8Decode(u8.ptr + off, strnlen(u8.ptr + off, NAME_MAX))
    const prefix = utf8Decode(u8.ptr + off + 345, strnlen(u8.ptr + off + 345, NAME_MAX))
