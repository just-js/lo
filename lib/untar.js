...
/**
 * @param {Uint8Array}  u8 - A Uint8Array containing a complete tarball
 * @param {number=} length - the length of the tarball. Default is the .length of u8
 * @returns {string} Top Level Directory name, or Undefined
 */
function untar (u8, buf_size = u8.length) {
  ...
  while (size) {
    ...
    while (todo > 0) {
      ...
      todo -= bytes
      if (fd && todo === 0) {
        const written = write(fd, u8.ptr + file_start, file_size)
